...
executable abstract machine in Lean 4 — the normative, machine-checkable definition of the protocol's
core handlers and state transitions. The [/spec pages](/spec) are the prose specification: the
human-readable companion that explains the same protocol. Where prose and Lean disagree, the Lean
model wins.

...
The two canonical messages flowing *to* your worker are **Invoke** ("start executing this") and **Resume** ("the thing you awaited is ready"). Recognizing and dispatching those is the heart of the worker loop.
...
Five words that the rest of the handbook leans on: **promise** (the durable value), **task** (the claim on it, with its version and lease), **message** (the enveloped request/response), **Invoke / Resume** (the two messages that drive a worker), and **suspend / fulfill** (how a worker yields and completes). With those in hand, [Talking to the server](/talking-to-the-server) starts the client — the connection, the transport, and the first real requests over the wire.

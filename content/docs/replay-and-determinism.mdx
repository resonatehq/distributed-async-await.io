...
The flip side, which your SDK should make legible: a step that is *not* durable (an SDK may allow opting out for inexpensive, side-effect-free work) is not replay-safe. It re-runs on every replay. That's a fine, deliberate optimization for a pure helper, and a quiet bug if a developer reaches for it around a side effect. Make the durable path the default and the non-durable path the conscious choice.

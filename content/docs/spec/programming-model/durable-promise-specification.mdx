...
<Callout type="note" title="Scope: Lean-modeled core vs. extended surface">
This page specifies the full durable promise contract, including surface the Lean 4 abstract machine ([resonatehq/resonate-specification](https://github.com/resonatehq/resonate-specification)) does not model: the `ikc`/`iku` idempotency keys, the `strict` flag, and the state-transition table built on them. That extended surface comes from the legacy durable-promise contract and is implemented by the reference server. Where this page and the Lean model overlap and disagree, the Lean model wins.
</Callout>

...
The protocol's [task model](/sdk/tasks-and-the-worker-loop) names a set of invariants the server maintains over every task at all times. They are the most useful thing to test against, because they are precise, they are total (they hold after *every* operation, not just at the end), and they translate directly into assertions. The [specification's task model](https://docs.resonatehq.io/spec/system-model/tasks) states seven:
...
- **Hand-written transition scenarios** that target specific tricky paths: the [`300`-continue fast path](/sdk/suspend-resume-settlement), a lease expiring mid-step, an idempotent re-create, a suspend that races a settlement. Each is a small, named, deterministic sequence whose expected end state you assert exactly.
...
Next: [production concerns](/sdk/production-concerns) — what running the SDK you built actually demands, from observability to versioning functions while work is in flight.

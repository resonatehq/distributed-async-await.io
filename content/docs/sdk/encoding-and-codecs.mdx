...
- **`headers`** is a string→string map that travels *alongside* `data` and tells the codec how to read it — content type, encoding, format markers. This is the forward reference the [promise lifecycle](/sdk/promise-lifecycle) chapter left open: headers exist so that decoding doesn't have to guess.
...
It is tempting to present encryption as a solved, uniform feature. It is not, and the handbook's habit of [surfacing the hard parts honestly](/sdk/introduction) applies — so here is exactly what ships.
...
Next: [local mode for development](/sdk/local-mode) — running the whole engine in-process so a developer can build against it with nothing else installed, and the one emulation gap you have to be honest about.

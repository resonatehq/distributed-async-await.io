...
The thing to hold onto: when a durable function awaits an unsettled promise, the worker does **not** sit on a thread waiting. It tells the server "I'm waiting on this," lets go of the execution entirely, and goes and does other work. The server remembers, and pushes the function back to a worker when the awaited promise settles. The wait costs nothing while it lasts — which is what makes a week-long sleep or a paused human-approval step as inexpensive as a fast one.

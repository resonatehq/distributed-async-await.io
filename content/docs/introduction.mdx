...
"Almost exactly" is doing a lot of work in that sentence, and most of this handbook lives inside it. The developer writes:
...
If wire-correctness were the whole job, an SDK would be a thin HTTP client and this would be a short document. It is not, because a library that is merely correct is not one people reach for.

Idiomatic means the durable function reads like a normal function in the host language. In a language with `async`/`await`, durability should ride on `await`. In a language built on generators or coroutines, it should ride on `yield`. In a language with futures, it should compose with futures. The reference SDKs already span two quite different shapes of this — a sync, generator-based model and an async, future-based one — and a new SDK's first real design decision is which shape fits the host language's grain.

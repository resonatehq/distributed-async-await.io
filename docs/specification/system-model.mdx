...
A distributed system is a collection of concurrent components ([processes](#processes)) that communicate by exchanging messages over the network.
...
A _term_ event may denote both a normal planned termination, or an abnormal unplanned termination ([fail-stop](#failure)).
